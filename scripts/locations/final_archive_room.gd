...
const DIALOGUE_PATH := "res://data/dialogue/chapter_05_archive.json"
const D51_REQUIRED := ["E501", "E502", "E503", "E504"]
...
func _begin() -> void:
	if not GameState.get_flag("final_archive_intro_seen", false):
...
func _on_interaction_requested(target: Investigable) -> void:
	match target.name:
		"DeductionBoard": _try_d51()
		"AgencyDoor":
			if GameState.get_flag("deduction_d51", false):
				_show_conversation("enter_agency", "goto_agency")
			else:
				_show_conversation("agency_locked")
...
func _try_d51() -> void:
	if not _has_all(D51_REQUIRED):
		_show_conversation("d51_not_ready")
		return
	hud.show_deduction("推理 · 同一记号的四封信", "选择能同时比较纸张、笔迹、折法与形成顺序的四份原件。", PackedStringArray(D51_REQUIRED), "D51")


func _on_deduction_finished(context: String) -> void:
	if context != "D51": return
	GameState.set_flag("deduction_d51")
	_show_conversation("d51_success", "d51_success")
	_update_objective()


func _on_dialogue_finished(context: String) -> void:
	match context:
		"intro": GameState.set_flag("final_archive_intro_seen")
		"goto_agency":
...
func _update_objective() -> void:
	if GameState.get_flag("deduction_d51", false):
		hud.set_objective("四封信来自不同作者：前往七码头追查交接路线")
	else:
		var found := 0
		for evidence_id: String in D51_REQUIRED:
			if GameState.has_evidence(evidence_id): found += 1
		hud.set_objective("比较四章委托原件：已找到 %d / 4" % found)
