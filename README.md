...
新玩家从第一章开始，每章达成结局后解锁下一章；后续章节要求此前各章均有通关记录。章节选择显示当前档位已达成的结局，已解锁章节可以重玩。第五章读取这个档位中前四章的实际结果。《索引》条件会在终幕明确显示，不使用隐藏善恶值。旧版存档自动迁入存档 1，原始文件保留；旧跳章存档仍可读取，但不会凭空补齐此前章节的通关记录。

## 当前完成情况

- Godot 4.6.2 项目、标题与章节选择、最多三个独立存档槽；各档分别保存线索、跨章结局和解锁状态；
...
- `Tab`：打开或关闭调查笔记；
- `Esc`：关闭当前界面，或打开暂停与设置；
- 标题“读取存档”：查看三个档位的章节、地点、保存时间和线索数，选择读取；
- 暂停菜单“保存调查”/“读取存档”：手动存读档，覆盖已有档位会先确认；保存到哪个档位，后续自动保存就更新哪个档位；
- 每个档位旁的“删除”可删除指定存档，需再次确认；删除当前档后游戏可继续，但必须手动另存才能恢复自动保存，其他档不受影响；
- “新调查 · 第一章”：选择空档位开始，也可确认覆盖已有档位；各档独立，最多同时保留三个档；
- 章节专用界面：按提示试听磁带、排列事件、叠入图层或选择材料类别。

每个档位同时承担该路线的自动保存。若要保留某个分支点，先暂停并另存到另一个档位，再继续游玩。手动保存会记录主角水平位置；对白、选择和谜题界面需先结束/关闭，再打开暂停菜单存档，不保存某句语音的播放秒数。详见 [`docs/SAVE_SYSTEM.md`](docs/SAVE_SYSTEM.md)。
